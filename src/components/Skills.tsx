...
const skills = [
  ...
  { name: "Azure AS", icon: <VscAzure size={30} className="text-blue-600" /> },
  { name: "Azure VM", icon: <VscAzure size={30} className="text-blue-600" /> },
  { name: "Azure Blob", icon: <VscAzure size={30} className="text-blue-600" /> },
  { name: "Firebase", icon: <SiFirebase size={30} className="text-orange-500" /> },
  { name: "CI/CD", icon: <IoInfiniteOutline size={30} className="text-gray-500" /> },
  { name: "MVC Architecture", icon: <PiTreeStructure size={30} className="text-blue-500" /> },
  { name: "RBAC", icon: <GoShieldLock size={30} className="text-blue-500" /> },
  { name: "CRON Jobs", icon: <FaNodeJs size={30} className="text-green-600" /> },
  { name: "MSC Architecture", icon: <PiTreeStructure size={30} className="text-blue-500" /> },
...
export default function Skills() {
  return (
    <section
    id="skills"
    className="py-12 md:py-20 bg-violet-100 rounded-lg shadow-lg shadow-violet-400 my-10 overflow-hidden"
  >
    <div className="container mx-auto px-6 md:px-10">
      <h2 className="text-5xl md:text-8xl font-bold text-center mb-6 md:mb-10 text-violet-200 pb-3 md:pb-5"
        data-aos="fade-up"> 
        Skills & Technologies </h2>
      <div
        data-aos="fade-up"
        className="grid grid-cols-2 md:flex md:flex-wrap justify-center gap-3 md:gap-4 cursor-default"
      >
          {skills.map((skill, index) => (
            <div
              key={index}
              className="group relative flex items-center px-1 md:px-5 py-1 md:py-3 bg-white shadow-md rounded-full space-x-3
      border-2 border-purple-500 overflow-hidden transition-all duration-300 cursor-pointer"
            >
              ...
              <span className="relative flex items-center font-bold transition-colors space-x-2 md:space-x-3 duration-200 ease-in-out group-hover:text-white">
